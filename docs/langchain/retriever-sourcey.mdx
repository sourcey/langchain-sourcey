...
`SourceyRetriever` is the retrieval layer behind an Ask AI flow over a Sourcey
docs site. It reads `search-index.json`, hydrates from `llms-full.txt`, and
returns `Document` objects with canonical page URLs in `metadata["source"]`.
